<template>
    <section
        class="px-4 py-6 bg-gray-800 rounded-xl shadow-md text-white grid gap-4"
        :class="gridClass"
        @click="$emit('create')"
    >
        ...
        <div v-if="!selectedAccount && savingAccountsValue > 0" class="text-center px-1">
            <h2 class="text-sm font-medium text-gray-400">Savings</h2>
            <p class="text-lg font-semibold text-white">
                {{ formatCurrency(savingAccountsValue, currency) }}
            </p>
        </div>
        <div v-if="!isSavings" class="text-center px-1">
