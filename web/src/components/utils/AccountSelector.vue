<template>
	<div v-if="isLogged && accounts.length" class="relative inline-block text-center text-white">
		...
		<div v-if="isDropdownOpen"
			...
			<div v-for="account in accounts" :key="account.name" class="px-4 py-2 hover:bg-gray-700 cursor-pointer"
	...
	<button v-else-if="isLogged" @click="isAccountModalOpen = true"
...
function addAccount(name: string) {
	...
	emit('add', { name, type: 'broker' } as Account)
	isDropdownOpen.value = false
